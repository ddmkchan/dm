...
public class MySimpleLDA implements Serializable {
	...
	public MySimpleLDA (int numberOfTopics) {
		this (numberOfTopics, numberOfTopics, DEFAULT_BETA);
	}
	
	public MySimpleLDA (int numberOfTopics, double alphaSum, double beta) {
	...
	public MySimpleLDA (int numberOfTopics, double alphaSum, double beta, Randoms random) {
		this (newLabelAlphabet (numberOfTopics), alphaSum, beta, random);
	}
	
	public MySimpleLDA (LabelAlphabet topicAlphabet, double alphaSum, double beta, Randoms random)
	{
		...
		this.alpha2 = new double[numTopics];//参考ParallelTopicModel ，分配到各个主题的概率
		Arrays.fill(alpha2, alphaSum / numTopics);//ParallelTopicModel
		this.beta = beta;
		this.random = random;
...
//		System.out.println("beta: "+beta+" alpha: "+alpha);//add
		...
		for (Instance instance : training) {
			...
			for (int position = 0; position < tokens.size(); position++) {
				...
				int type = tokens.getIndexAtPosition(position);//该特征词在词表alphabet中的下标
...
//				tmp2.append(t + " ");
//			}
//			System.out.println("term "+"to topics\t"+tmp2.toString());
...
//			System.out.println("before sample\ttokensPerTopic: \t "+position);
//			StringBuilder tmp = new StringBuilder();
//			for (int j:tokensPerTopic) {
//				tmp.append(j + " ");
//			}
...
//			System.out.println("old Topic " + oldTopic + " below 0");
...
//		alpha = in.readDouble();
	...
	public static MySimpleLDA read (File f) throws Exception {

		MySimpleLDA topicModel = null;

		ObjectInputStream ois = new ObjectInputStream (new FileInputStream(f));
		topicModel = (MySimpleLDA) ois.readObject();
		ois.close();
